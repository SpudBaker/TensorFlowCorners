...
export class ClassificationPage implements OnInit {
  ...
  classNames: any = ['Zero', 'One', 'Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine'];
  model: any;
  ...
  async run() {
    ...
    await this.train(this.model, data);
  ...
  async showExamples(data) {
    ...
    for (let i = 0; i < numExamples; i++) {
      ...
      can.width = 28;
      can.height = 28;
      can.style = 'margin: 4px;';
      await tf.browser.toPixels(imageTensor, can);
      surface.drawArea.appendChild(can);
      imageTensor.dispose();
    }
  }
